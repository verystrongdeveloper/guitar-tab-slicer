...
export const ALLOWED_SCORE_EXTENSIONS = new Set([
  '.gp',
  '.gp3',
  '.gp4',
  '.gp5',
...
function normalizeOriginalName(originalName) {
  const name = path.basename(String(originalName || 'score').trim() || 'score');
  return name.replace(/[\r\n"]/g, '_');
}

export function validateScoreInput(buffer, originalName = 'score') {
  ...
  return { buffer: normalizedBuffer, originalName: safeName };
}

export function createAlphaTexScoreInput(alphaTex) {
  const text = String(alphaTex || '').trim();
  if (!text) {
    throw new Error('Paste AlphaTex text first.');
  }
  if (text.length > MAX_ALPHA_TEX_CHARS) {
    throw new Error(`AlphaTex text is too large. Maximum length is ${MAX_ALPHA_TEX_CHARS} characters.`);
  }
  return validateScoreInput(Buffer.from(text, 'utf8'), 'input.alphatex');
}

export function analyzeScoreBuffer(buffer, originalName = 'score') {
  const file = validateScoreInput(buffer, originalName);
  const score = loadScoreFromUpload(file.buffer);
  return scoreToMetadata(score);
}

export async function renderScoreZipBuffer(buffer, originalName = 'score', body = {}) {
  const file = validateScoreInput(buffer, originalName);
  ...
  return {
    buffer: zipBuffer,
    filename: `${baseName || 'tab'}_overlay_slices.zip`,
    metadata,
    selectedTracks: trackIndexes,
    options,
    chunks
